...
#     if ($3 >= 30) print $1
#   }'))
DISKS=($(lsblk -dno NAME,TYPE | awk '$2!="rom" {print $1}'))
# if [ ${#DISKS[@]} -eq 0 ]; then
...
for i in "${!DISKS[@]}"; do
    DEVINFO=$(lsblk -dno TYPE,SIZE,MODEL /dev/${DISKS[$i]} 2>/dev/null)
    TYPE_COL=$(echo "$DEVINFO" | awk '{print $1}')
    SIZE=$(echo "$DEVINFO" | awk '{print $2}')
    MODEL=$(echo "$DEVINFO" | awk '{$1=$2=""; sub(/^ +/,"",$0); print $0}')
    [ -z "$MODEL" ] && MODEL="Unknown"
    echo -e "  ${CYAN}[$i]${SL_RESET} /dev/${DISKS[$i]} - $SIZE - $TYPE_COL - $MODEL"
done
# for i in "${!DISKS[@]}"; do
#   echo " [$i] /dev/${DISKS[$i]}"
#   lsblk -d /dev/${DISKS[$i]}
